...
import com.icaboalo.yana.old.ui.activity.LoadingActivity;
...
public class SplashScreenActivity extends BaseActivity {
    ...
    private void checkForToken(){
        if (PrefUtils.getToken(getApplicationContext()).isEmpty() || PrefUtils.getToken(getApplicationContext()).equals("TOKEN")){
            navigator.navigateTo(getApplicationContext(), new Intent(getApplicationContext(), TutorialActivity.class));
            finish();
        } else {
            if (!PrefUtils.isDownloadCompleted(getApplicationContext())){
                navigator.navigateTo(getApplicationContext(), new Intent(getApplicationContext(), LoadingActivity.class));
                finish();
            }
            else {
                navigator.navigateTo(getApplicationContext(), new Intent(getApplicationContext(), MainActivity.class));
                finish();
            }
        }
    }
